...
describe('catalog tools', () => {
  ...
  it('otwa_list_iso_library GETs /v1/iso-library', async () => {
    const client = new FakeOtwaClient().on('GET /v1/iso-library', [
      { id: 'a1b2', name: 'mikrotik-7.22', ready: true },
    ]);
    const { server, handlers } = captureHandlers();
    registerCatalogTools(server, asOtwaClient(client));

    await handlers['otwa_list_iso_library']!({});
    expect(client.calls[0]?.path).toBe('/v1/iso-library');
  });
});
